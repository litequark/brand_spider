...
class BasePage:
    ...
    def find_element(self, locator, timeout=2):
        ...
        try:
            ...
            self.logger.info(f"查找元素超时: {locator}")
            raise
    ...
    def find_elements(self, locator, timeout=None, visible: bool = False):
        ...
        try:
            ...
            self.logger.info(f"查找元素集合超时: {locator}")
            return []  # 返回空列表而非抛出异常

    def click(self, locator: WebElement | tuple[str, str], timeout: float = None) -> None:
        """点击元素（带等待元素可点击）"""
        timeout = timeout or self.timeout
        try:
    ...
    def scroll_to_element(self, locator: tuple[str, str] | WebElement) -> None:
        """滚动到指定元素位置"""
        if isinstance(locator, tuple):
            ...
            error_log: str = f'Invalid locator type: {type(locator).__name__}'
    ...
    def scroll_to_contained_element(self, target: tuple[str, str] | WebElement,
                                    container: tuple[str, str] | WebElement):
